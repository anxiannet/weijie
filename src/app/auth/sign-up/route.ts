...
import {createAuthRouteClient, getSiteUrl, redirectTo, requireAuthString, toAuthMessage} from '../auth-utils';
...
export async function POST(request: NextRequest) {
  ...
  return redirectTo(request, '/auth?message=账户已创建。请在 Supabase 关闭邮箱确认后，即可注册后直接登录。');
}
